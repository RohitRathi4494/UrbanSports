...
function seedDatabase(db: Database.Database) {
  ...
  const insertProduct = db.prepare(`
    INSERT INTO products (id, name, slug, category_id, brand, sku, description, features, specifications, price, mrp, images, stock, is_featured, is_new_arrival, tags, meta_title, meta_description)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
